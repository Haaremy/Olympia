...
interface Entry {
  ...
  game: {
    id: number;
    languages: {
      title: string;  // Ensure 'title' is fetched here
    }[];
    tagged: string | null;
    points: GamePoint[];
  };
  ...
  try {
      ...
      include: {
        game: {
          ...
          include: {
            languages: {
              select: {
                title: true,  // Ensure we are fetching the 'title' from the 'languages' relation
              },
            },
          },
        },
    ...
    for (const gameId in groupedByGame) {
      ...
      result.push({
        ...
        gameName: firstEntry.game.languages[0]?.title || "",  // Ensure safe access to 'title'
