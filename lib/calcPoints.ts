// lib/calculatePoints.ts

export interface PointsInput {
  game: number;
  field: number;
  userPoints: number;
  multiplier: number;
}

export interface PointsResult {
  result: number;
  cheats: number;
}

/**
 * Beispiel: Summiert alle Punkte und gibt die Differenz zwischen User1 und User2 zurück.
 */
export function calculatePoints({ game, userPoints, multiplier, field }: PointsInput): {result: number; cheats: number} { // spiel, punkte im feld, spielerzahl ausgleich (2 vs 4 spieler team), feld zur Auswertung
  let result = 0;
  let cheats = 0;

  ////////////////////////////
  if (game==1){
    if(field==1 && userPoints==850){
      result=10;
    }
    if(field==2 && userPoints==339){
      result=10;
    }
    if(field==3 && userPoints==976){
      result=10;
    }
    if(field==4 && userPoints==398){
      result=10;
    }
  }
  //////////////////////////
////////////////////////////
  if (game==2){
    if(field==1 && userPoints==357){
      result=10;
    }
    if(field==2 && userPoints==179){
      result=10;
    }
    if(field==3 && userPoints==915){
      result=10;
    }
    if(field==4 && userPoints==741){
      result=10;
    }
  }
  ////////////////////////////
  ////////////////////////////
   if (game==3){
    if(field==1 && userPoints==5){
      result=10;
    }
    if(field==2 && userPoints==6){
      result=10;
    }
    if(field==3 && userPoints==20){
      result=10;
    }
    if(field==4 && userPoints==8){
      result=10;
    }
  }
  ////////////////////////////
  ////////////////////////////[70, 216, 201, 110]
  if (game==4){
    if(field==1 && userPoints==70){
      result=10;
    }
    if(field==2 && userPoints==216){
      result=10;
    }
    if(field==3 && userPoints==201){
      result=10;
    }
    if(field==4 && userPoints==110){
      result=10;
    }
  }
  ////////////////////////////
  ////////////////////////////    [1, 1, 1, 1]
  if (game==5){
    if(field==1 && userPoints==1){
      result=10;
    }
    if(field==2 && userPoints==1){
      result=10;
    }
    if(field==3 && userPoints==1){
      result=10;
    }
    if(field==4 && userPoints==1){
      result=10;
    }
    result=result*multiplier;
  }
  ////////////////////////////
  ////////////////////////////
  if (game==6){
    result=userPoints*multiplier;
    if(userPoints>10){
      result=0;
      cheats=1*multiplier;
    }
  }
  //////////////////////////// Spiel auf Zeit in Sekunden
  //////////////////////////// [9, 9.5, 10, 10.5, 11, 12, 13, 15, 17, 20]
  if (game==7){
    // GAME 7 – Zeitstaffelung korrekt
    if (userPoints < 10) {
        result = 0;
        cheats = 1;
    }
    else if (userPoints < 15) result = 10;
    else if (userPoints < 16) result = 9;
    else if (userPoints < 17) result = 8;
    else if (userPoints < 18) result = 7;
    else if (userPoints < 19) result = 6;
    else if (userPoints < 20) result = 5;
    else if (userPoints < 21) result = 4;
    else if (userPoints < 22) result = 3;
    else if (userPoints < 23) result = 2;
    else if (userPoints < 24) result = 1;
    else result = 0;

  }
//////////////////////////// Schätzen mit Abweichung
////////////////////////////[72, 3534, 99, 115]
 // SCHÄTZSPIEL (8 oder 13)
if (game == 8 || game == 13) {

    let check = 0;

    if (game == 8) {
        if (field == 1) check = 144;
        if (field == 2) check = 232;
        if (field == 3) check = 177;
        if (field == 4) check = 2000;
    }

    if (game == 13) {
        if (field == 1) check = 59871;
        if (field == 2) check = 17471;
        if (field == 3) check = 25951;
        if (field == 4) check = 33695;
    }

    const diff = Math.abs(userPoints - check);

    if (diff == 0) {
        result = 10;
    }
    else if (diff <= check * 0.1) {
        result = 9;
    }
    else if (diff <= check * 0.2) {
        result = 8;
    }
    else if (diff <= check * 0.3) {
        result = 7;
    }
    else if (diff <= check * 0.4) {
        result = 6;
    }
    else if (diff <= check * 0.5) {
        result = 5;
    }
    else if (diff <= check * 0.6) {
        result = 4;
    }
    else if (diff <= check * 0.7) {
        result = 3;
    }
    else if (diff <= check * 0.8) {
        result = 2;
    }
    else if (diff <= check * 0.9) {
        result = 1;
    }
}

////////////////////////////
////////////////////////////
    if (game==9){
    result=userPoints*2*multiplier; // treffer * wertung pro treffer (2P pro Treffer) * Ausgleich
      if(userPoints>5){
      result=0;
      cheats=1*multiplier;
    }
  }
  ////////////////////////////
  //////////////////////////// stand 2025
    if (game==10){
    if(field==1 && userPoints==13){ // elias 6 + spekulatius 7 = 13
      result=10;
    }
    if(field==2 && userPoints==14){ // fiona 9 + lebkuchen 5 = 14
      result=10;
    }
    if(field==3 && userPoints==24){ // greta 7 + zimtstrerne 17 = 24
      result=10;
    }
    if(field==4 && userPoints==26){ // hannes 5 + vanillekipferl 11 = 26
      result=10;
    }
      
  }
  ////////////////////////////
  ////////////////////////////
    if (game==11){
    result=userPoints*multiplier;
      if(userPoints>10){
      result=0;
        cheats=1*multiplier;
    }
  }
  ////////////////////////////
  ////////////////////////////
    if (game==12){
    result=userPoints*multiplier;
      if(userPoints>10){
      result=0;
      cheats=1*multiplier;
    }
  }
  //////////////////////////// 
  //////////////////////////// schätzen mit abweichung stand 2025
    // Spiel 13 siehe Spiel 8
  ////////////////////////////
  //////////////////////////// stand 2025
    if (game==14){
        if(field==1 && userPoints==5){
      result=10;
    }
    if(field==2 && userPoints==3){
      result=10;
    }
    if(field==3 && userPoints==1){
      result=10;
    }
    if(field==4 && userPoints==2){
      result=10;
    }

  }
  ////////////////////////////
  ////////////////////////////
    if (game==15){
        result=userPoints*multiplier;
      if(userPoints>10){
      result = 10*multiplier;
    }
      if(userPoints>15){
      result = 0;
        cheats=1*multiplier;
    }
  }
  ////////////////////////////
  ////////////////////////////
    if (game==16){
        result=userPoints*multiplier;
      if(userPoints>10){
      result = 0;
        cheats=1*multiplier;
    }

  }
  ////////////////////////////
  ////////////////////////////
    if (game==17){
        result=userPoints*multiplier;
      if(userPoints>10){
      result = 0;
        cheats=1*multiplier;
    }

  }
  ////////////////////////////
  ////////////////////////////
    if (game==18){
       if(field==1 && userPoints==15){
      result=10;
    }
    if(field==2 && userPoints==26){
      result=10;
    }
    if(field==3 && userPoints==37){
      result=10;
    }
    if(field==4 && userPoints==49){
      result=10;
    }
  }
  ////////////////////////////
  ////////////////////////////
    if (game==19){
        result=userPoints*multiplier;
      if(userPoints>10){
      result = 0;
        cheats=1*multiplier;
    }

  }
  ////////////////////////////
  ////////////////////////////
    if (game==20){
      if (field==1){
        if (userPoints < 10) {
            result = 0;
            cheats = 1;
        }
        else if (userPoints < 13) result = 10;
        else if (userPoints < 14) result = 9;
        else if (userPoints < 15) result = 8;
        else if (userPoints < 16) result = 7;
        else if (userPoints < 17) result = 6;
        else if (userPoints < 18) result = 5;
        else if (userPoints < 19) result = 4;
        else if (userPoints < 20) result = 3;
        else if (userPoints < 21) result = 2;
        else if (userPoints < 22) result = 1;
        else result = 0;

      }
}

  ////////////////////////////
  ////////////////////////////[2345, 4535, 3265, 4542]
    if (game==21){
     if(field==1 && userPoints==2345){
      result=10;
    }
    if(field==2 && userPoints==4535){
      result=10;
    }
    if(field==3 && userPoints==3265){
      result=10;
    }
    if(field==4 && userPoints==4542){
      result=10;
    }
  }
  ////////////////////////////
  ////////////////////////////
    if (game==22){
      result=userPoints*2*multiplier;
      if(userPoints>5){
      result = 0;
        cheats=1*multiplier;
    }

  }
  ////////////////////////////
  ////////////////////////////
    if (game==23){
      result=userPoints*2*multiplier;
      if(userPoints>5){
          result = 0;
        cheats=1*multiplier;
        }
    }
  ////////////////////////////
  ////////////////////////////[30, 35, 40, 45, 50, 55, 60, 65, 70, 75], [198, 7, 499]
    if (game==24){
      if (field==1){
        if (userPoints < 20) {
            result = 0;
            cheats = 1;
        }
        else if (userPoints < 30) result = 10;
        else if (userPoints < 35) result = 9;
        else if (userPoints < 40) result = 8;
        else if (userPoints < 45) result = 7;
        else if (userPoints < 50) result = 6;
        else if (userPoints < 55) result = 5;
        else if (userPoints < 60) result = 4;
        else if (userPoints < 65) result = 3;
        else if (userPoints < 70) result = 2;
        else if (userPoints < 75) result = 1;
        else result = 0;

      } 
      if(field==2 && userPoints==441){
        result=10;
      }
      if(field==3 && userPoints==938){
        result=10;
      }
      if(field==4 && userPoints==375){
        result=10;
      }
  }

  ////////////////////////////
  
  if(userPoints<=-1){
    cheats = 0;result = 0;
  }
  return {result, cheats};
}
